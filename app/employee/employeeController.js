    ...
    async insertEmployee(req, res) {
        ...
        if (req.user) {
            ...
            if (req.body.position == 2) {
                let emp = await EmployeeModel.getManagerUsername(req.body)
                if (emp.length > 0) {
                    console.log('managerซ้ำ')
                    res.status(201).json({
                        result: { emp: false }
                    });
                } else {
                ...
                let emp = await EmployeeModel.getEmployeeUsername(req.body)
                if (emp.length > 0) {
                    console.log('employeeซ้ำ')
                    res.status(201).json({
                        result: { emp: false }
                    });
                } else {
