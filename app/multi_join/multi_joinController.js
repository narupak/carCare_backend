...
const Multi_joinController = {
    ...
    async getDetailCarByMemberApi(req, res) {
        let detailCar;
        let queueMember = await Multi_joinModel.getQueueForDateAndMember(moment(new Date()).format('YYYY-MM-DD'))
        if (queueMember.length > 0) {
            for (let i = 0; i < queueMember.length; i++) {
                detailCar = await Multi_joinModel.getDetailCarByMemberANDCar(queueMember[i].car_detail_id);
            }
        } else {
